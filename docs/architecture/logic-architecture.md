...
### 1.3 布局架构 (Layout Shell)
全站采用全屏固定布局 (Fixed Layout)，通过 CSS 隔离实现各区域独立交互：
1. Body 容器：应用 `h-screen overflow-hidden`，禁绝全局原生滚动。
2. 独立滚动区域：
   - MemoEditor：固定在中间栏顶部。
   - MemoFeed：中间栏的主体内容区，应用独立滚动逻辑。
   - Sidebars：左右侧边栏通过 Sticky 或独立滚动层，互不干扰，不随页面主逻辑偏移。
3. 滚动监听 (Scroll Spy)：右侧 Timeline 通过对中间栏 MemoFeed 的滚动进度监听，实现月份节点的实时高亮与联动跳转。
...
2.  性能驱动的动画: `MemoEditor` 放弃了全局的 `layout` 投影，转而使用内部显式的 `animate` 属性控制 `minHeight`。通过 `isAnimating` 状态管理，在动画期间动态切换 `overflow` 和 `mask-image` 表现，确保内容在展开/收缩过程中不产生形变。
3.  高度状态管理: 编辑器支持“折叠、默认、最大”三种高度预设。折叠状态下强制 `h-[60px]` 并隐藏所有功能组件。
4.  联动更新：利用 `useStats` 和 `useTags` 的 Hook，在发布成功的回调里瞬间通知全站 Sidebar 更新。
