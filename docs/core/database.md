...
热力图、时间轴和导出相关结果目前不是单一地全部放在数据库函数里，也不是单一地全部放在 Server Action 层。

当前更准确的分工是：

* 公共标签聚合这类稳定口径，使用数据库函数完成
* 热力图、时间轴和导出这类需要结合当前查看者身份裁剪的结果，主要在 Server Action 层完成

原因是这些结果需要结合当前查看者身份进行裁剪：
