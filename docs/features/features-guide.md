...
- **三种独立浏览模式**:
    ...
    2. **时间轴模式 (Timeline Context Teleport)**: 在普通首页往下无限翻时，点击右侧时间轴的某一年、月、日。页面 **URL 不变**，执行原地时空跳跃。
        - 列表瞬间清屏，加载所选时间维度的最新数据（如：按月点击则加载该月最新的 20 条，往下无限滚动仅限于该月份内）。
        - **Calendar Pager 导航器**: 在列表的顶部和底部分别生成优雅的分级翻页指示器。
            - 选“日”：显示 `[View Newer memos (最近的有数据的下一日)]` / `[View Older memos (最近的上一日)]`。
            - 选“月”：显示 `[View Newer memos (最近的有数据的下一月)]` / `[View Older memos (最近的上一月)]`。
            - 选“年”：显示 `[View Newer memos (最近的有数据的下一年)]` / `[View Older memos (最近的上一年)]`。
    3. **标准瀑布流模式 (Infinite Scroll)**: 在没有任何日期参数、也不处于特定时间段翻页上下文时，首页默认行为为极其顺滑的单向（向下）全局无限瀑布流加载。
- **并发探针技术 (Cursor Probing)**: 
    - 支撑“层级翻页模式”的核心 API 升级为 `getContextMemosWithNeighbors`。
    - 当用户传送到某一天/月/年时，后端并发执行 3 组查询：抓取当前时间维度的数据（分页加载）、利用 `limit(1)` 获取**实际拥有数据**的最近的上一个同级时间维度（作为 View Older 游标）、获取**实际拥有数据**的最近的下一个同级维度（作为 View Newer 游标）。杜绝空跳。
