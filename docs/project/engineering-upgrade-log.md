...
## 2026-03-10: 安全性加固与 UI 性能标准建立
- **重构背景**：
  - 发现 RPC 在搜索模式下仍会泄露私密笔记的元数据（ID、标签等）。
  - 数据库字段与应用层命名不一致（`access_code` vs `access_code_hash`）。
  - RLS 限制了匿名用户在服务端验证口令哈希的能力。
- **架构演进**：
  - **隐私深度脱敏**：重构 `search_memos_secure` RPC。现在，对于未授权的私密笔记，在搜索/过滤模式下执行“物理抹除”（完全不返回数据行），仅在默认 Feed 视图下保留锁定占位符。
  - **权限提升模型**：在 `verifyUnlockCode` 中引入 `getAdminClient` (Service Role)，实现了在受控环境（服务端）下安全绕过 RLS 校验哈希，解决了匿名用户无法解锁私密笔记的逻辑死锁。
  - **字段命名统一**：全量迁移数据库字段为 `access_code_hash`，并同步更新了所有迁移文件与种子数据。
  - **UI 动画基准 (Baseline)**：在工程标准中确立了高性能动画规范，强制要求使用合成层属性，并推荐使用 `layoutId` 处理布局变换。
- **安全性提升**：
  - 修复了 RPC 的 `search_path` 安全漏洞，防止搜索路径劫持攻击。
  - 统一了前后端加密标准为 `$2a$` (bcrypt)，确保了哈希校验的 100% 兼容性。
