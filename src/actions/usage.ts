...
const buildMetric = (used: number, limit: number, unit: string): UsageMetric => ({
    used,
    limit,
...
async function fetchManagementUsage(projectRef: string, apiKey: string): Promise<SupabaseUsageData | null> {
    try {
        ...
        const egressUsedBytes = Number(usage?.db_egress?.usage ?? 0) + Number(usage?.storage_egress?.usage ?? 0);

        return {
            db: buildMetric(bytesToMb(dbUsedBytes), FREE_PLAN_LIMITS.dbMb, "MB"),
            storage: buildMetric(bytesToMb(storageUsedBytes), FREE_PLAN_LIMITS.storageMb, "MB"),
            mau: buildMetric(mauUsed, FREE_PLAN_LIMITS.mau, ""),
            egress: buildMetric(bytesToGb(egressUsedBytes), FREE_PLAN_LIMITS.egressGb, "GB"),
            realtime: {
...
async function getDatabaseUsageMb() {
    ...
    const rpcResult = await (supabase as unknown as RpcCapableAdminClient).rpc("get_database_size");
...
async function getStorageUsageMb() {
    try {
        const supabase = getAdminClient() as unknown as StorageSchemaCapableAdminClient;
        const { data, error } = await supabase
            .schema("storage")
            .from("objects")
            .select("metadata");
...
export async function getSupabaseUsageStats(): Promise<SupabaseUsageResult> {
    ...
    if (projectRef && managementApiKey) {
        const managementData = await fetchManagementUsage(projectRef, managementApiKey);
