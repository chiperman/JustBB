...
import { getSupabaseUsageStats, type SupabaseUsageResult } from "@/actions/usage";
...
function MetricCard({
    ...
    return (
        <div className="rounded-card border border-gray-100 bg-white p-4 shadow-sm">
            ...
            <div className={cn("text-lg font-bold tabular-nums", muted ? "text-gray-400" : "text-gray-800")}>
                {value}
            </div>
...
export function UsageModal({ trigger }: UsageModalProps) {
    ...
    return (
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
            ...
            <DialogContent className="max-w-md overflow-x-hidden bg-white border-none p-0 overflow-y-hidden shadow-2xl rounded-3xl [&>button]:hidden">
                            ...
                            >
                                    ...
                                    <div className={cn(
                                        ...
                                        <div className="flex items-start gap-2.5">
                                            <div className={cn(
                                                ...
                                                error ? "bg-red-100 text-red-500" : "bg-orange-100 text-orange-500"
                                            )}>
                                                <HugeiconsIcon
                                            ...
                                            <div className="space-y-1">
                                                ...
                                                <p className="text-[11px] text-gray-400">
                                                    {error ? "已保留当前卡片结构，可手动重试。" : "数据返回后会直接填充到下方图表。"}
                                                </p>
                                            </div>
                                        </div>
                                        {error && (
                                            <Button onClick={() => void fetchData()} variant="outline" size="sm" className="rounded-md active:scale-95 shadow-sm">
                                                重试
                                            </Button>
                                        )}
                                    </div>
                                ...
                                <div className="flex items-center justify-between bg-orange-50/40 p-4 rounded-card border border-orange-100/50">
                                    <div className="flex items-center gap-2.5">
                                        <div className={cn("w-2 h-2 rounded-full", displayStats.isFullIndicator ? "bg-green-500" : "bg-orange-400")} />
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className={cn(
                                            ...
                                            {displayStats.isFullIndicator ? "Management API" : "Fallback"}
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div className={cn("space-y-6", loading && !stats && "opacity-70")}>
                                        ...
                                        <div className="grid grid-cols-2 gap-3">
                                            ...
                                            <div className="rounded-card border border-gray-100 bg-white p-4 shadow-sm">
                                                <div className="mb-1 flex items-center gap-1 text-[11px] font-medium text-gray-400">
                                                    <HugeiconsIcon icon={CheckListIcon} size={12} className={cn(isPlaceholder && "text-gray-300")} />
                                                    <span>数据完整性</span>
                                                    <InfoHint
                                                        label="数据完整性"
                                                ...
                                                <div className={cn(
                                                    ...
                                                    isPlaceholder ? "text-gray-400" : displayStats.isFullIndicator ? "text-green-600" : "text-orange-500"
                                                )}>
                                                    <span className={cn(
                                                        "h-1.5 w-1.5 rounded-full",
                                                        isPlaceholder ? "bg-gray-300" : displayStats.isFullIndicator ? "bg-green-500" : "bg-orange-400"
                                                    )} />
                                                    {displayStats.isFullIndicator ? "全量" : "基础"}
                                                </div>
                                                <div className={cn("mt-1 text-[11px]", isPlaceholder ? "text-gray-400" : "text-gray-400")}>
                                                    {displayStats.isFullIndicator ? "来自 Management API" : "部分指标为回退值"}
                                                </div>
                                            </div>
                                        </div>
                                    </section>
