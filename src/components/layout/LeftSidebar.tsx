...
export function LeftSidebar({ onClose, initialCollapsed = false }: LeftSidebarProps) {
    ...
    return (
        ...
        >
            ...
            <div
                className={cn(
                    "mb-5 flex min-h-9 transition-[margin,gap] duration-200",
                    effectiveIsCollapsed ? "flex-col items-center gap-1.5" : "items-center gap-2 pr-1"
                )}
            >
                <div className={cn("h-9 overflow-hidden", effectiveIsCollapsed ? "w-9 min-w-9 shrink-0" : "flex-1 min-w-0")}>
                ...
                >
                    <span className="flex items-center justify-center">
                        {isMobile ? (
                            ...
                            effectiveIsCollapsed ? <HugeiconsIcon icon={PanelLeftCloseIcon} size={16} /> : <HugeiconsIcon icon={PanelLeftOpenIcon} size={16} />
                        )}
                    </span>
                </Button>
            </div>
            ...
            <motion.div
                ...
                transition={
                    effectiveIsCollapsed
                        ? {
                            opacity: CONTENT_FADE_TRANSITION,
                            height: { duration: 0, delay: CONTENT_FADE_TRANSITION.duration },
                        }
                        : {
                            height: { duration: 0 },
                            opacity: CONTENT_FADE_TRANSITION,
