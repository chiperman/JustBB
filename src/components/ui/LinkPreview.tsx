...
export function LinkPreview({ url, customTitle, className, showCopyButton = false }: LinkPreviewProps) {
    ...
    if (!shouldLoad || loading) {
        return (
            ...
            >
                {showCopyButton && (
                    <div className="absolute top-2 right-2 opacity-0 group-hover/link-preview:opacity-100 transition-opacity bg-background/80 backdrop-blur-sm rounded-md border border-border/50 shadow-sm p-0.5 z-10">
    ...
    return (
        <div
            className={cn(
                "group/link-preview relative my-3 max-w-2xl overflow-hidden rounded-card border border-border bg-card transition-all hover:bg-accent/50 hover:shadow-md hover:border-primary/30",
