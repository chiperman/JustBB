  ...
  return (
    <div className="pt-4 pb-2 space-y-4 animate-in fade-in slide-in-from-top-2 duration-500 border-l-2 border-primary/5 pl-4 ml-1">
      ...
      <div className="flex flex-wrap gap-x-3 gap-y-1.5 opacity-80">
        ...
        <RequirementItem
          met={strength.metRequirements.upper && strength.metRequirements.lower}
          label="大小写"
        />
...
function RequirementItem({ met, label }: { met: boolean; label: string }) {
  return (
    <div className="flex items-center gap-1.5 group">
      <div
        className={`flex-shrink-0 w-3.5 h-3.5 rounded-full flex items-center justify-center transition-all duration-300 ${met ? "bg-green-500/10 text-green-600 scale-110-[0_0_8px_rgba(34,197,94,0.15)]" : "bg-foreground/[0.03] text-muted-foreground/20"}`}
      >
        {met ? (
          <HugeiconsIcon icon={Check} size={8} className="stroke-[3.5]" />
        ) : (
          <div className="w-0.5 h-0.5 rounded-full bg-current opacity-40" />
        )}
      </div>
      <span
        className={`text-[9px] font-bold uppercase tracking-wider transition-colors duration-300 ${met ? "text-green-600/80" : "text-muted-foreground/30"}`}
      >
        {label}
      </span>
    </div>
