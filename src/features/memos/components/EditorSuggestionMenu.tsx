...
export function EditorSuggestionMenu({
  ...
  if (!hasMounted || !position || (suggestions.length === 0 && !isLoading))
    return null
      ...
      <>
        {parts.map((part, i) =>
          part.toLowerCase() === q.toLowerCase() ? (
            <mark
              key={i}
              className="bg-primary/20 text-primary px-0.5 rounded-md font-medium"
            >
              {part}
            </mark>
          ) : (
  ...
  return createPortal(
    ...
    >
      <div className="bg-background border border-border/40 rounded-md overflow-hidden flex flex-col max-h-[450px]">
                ...
                {item.label.startsWith("#") ? (
                  ...
                  <>
                    ...
                    <div className="text-xs leading-relaxed text-foreground/80 break-words pr-2">
                      {item.subLabel &&
                        renderHighlightedText(item.subLabel, query)}
                    </div>
                  </>
                )}
              </li>
