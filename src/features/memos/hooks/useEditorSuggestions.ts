...
import { searchMemosForMention } from "@/server/actions/memos/query"
...
export function useEditorSuggestions({
  ...
  const fetchHashtagSuggestions = useCallback(async (query: string) => {
    ...
    if (SUGGESTION_CACHE.tags) {
      ...
      if (currentVersion === requestVersionRef.current) {
        ...
        const isFresh =
          Date.now() - SUGGESTION_CACHE.tags.timestamp <
          SUGGESTION_CACHE.CACHE_TTL
    ...
    // 2. 后台验证 (Revalidate)
    try {
      const { getAllTags } = await import("@/server/actions/memos/analytics")
      ...
      if (res.success && res.data) {
        ...
        if (currentVersion === requestVersionRef.current) {
          ...
          const exactMatch = freshFiltered.find(
            (f) => f.id.toLowerCase() === lowerQuery
          )
          if (!exactMatch && query.trim()) {
    ...
    async (query: string, offset: number = 0) => {
      ...
      if (isInitial) {
        ...
        if (cached) {
          ...
          const isFresh =
            Date.now() - cached.timestamp < SUGGESTION_CACHE.CACHE_TTL
          if (isFresh) {
            setIsLoading(false)
          } else {
            setIsLoading(true)
      ...
      const performFetch = async () => {
        ...
        try {
          const res = await searchMemosForMention(
            query,
            offset,
            20,
            unlockedMemoIds
          ...
          const newSuggestions = isInitial
            ? items
            : [
                ...suggestionsRef.current,
                ...items.filter(
                  (it) => !suggestionsRef.current.find((p) => p.id === it.id)
                ),
              ]
          ...
          // 2. 更新缓存 (Revalidate)
          if (isInitial) {
            SUGGESTION_CACHE.mentions.set(query, {
              data: items,
              timestamp: Date.now(),
            })
            // 保持 LRU 策略，避免 Map 过大
            if (SUGGESTION_CACHE.mentions.size > 50) {
              ...
              if (firstKey !== undefined)
                SUGGESTION_CACHE.mentions.delete(firstKey)
            }
          }
        } catch (err) {
  ...
  const handleSelectSuggestion = useCallback(
    (item: SuggestionItem, editor: Editor | null) => {
