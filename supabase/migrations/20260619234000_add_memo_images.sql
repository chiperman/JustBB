-- 20260619234000_add_memo_images.sql
-- 1. 添加 images 列
ALTER TABLE public.memos ADD COLUMN IF NOT EXISTS images TEXT[] DEFAULT '{}'::TEXT[] NOT NULL;

-- 2. 重写 search_memos_secure 函数，支持 images 字段及其未解锁状态脱敏，并采用 images 字段过滤图片
...
BEGIN
  ...
  SELECT
    ...
    b.owner_id,
    CASE
      -- 已解锁或所有人返回真实正文，未解锁返回空正文（因为图片已不在正文）
    ...
    b.locations,
    CASE
      -- 图片资源脱敏：已解锁返回真实图片，未解锁且含有图片时返回模糊锁定的占位图片
